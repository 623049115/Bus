...
@interface JDONearMapController () <UITableViewDataSource,UITableViewDelegate> {
...
- (void) loadData {
    ...
    for(int i=0; i<combinedStations.count; i++){
        // 从数据库查询该站点途径的线路
        // 若站点没有公交线路通过，则认为该站点无效，例如7路通过的奥运酒店
        JDOStationModel *station = combinedStations[i];
        ...
        for (int i=0; i<station.linkStations.count; i++) {
            JDOStationModel *linkStation = (JDOStationModel *)station.linkStations[i];
        ...
        if (station.passLines.count >0) {
            ...
            [station.passLines sortUsingComparator:^NSComparisonResult(id obj1, id obj2) {
...
//        NSString *key = [NSString stringWithFormat:@"%@[%@]",station.name,station.direction];
//        JDOStationModel *mapStation2 = (JDOStationModel *)[sameNameStationMap objectForKey:key];
//        if (mapStation1) {
...
//    // ===============test===============
////    JDOStationAnnotation *pointAnnotation = [[JDOStationAnnotation alloc] init];
////    pointAnnotation.coordinate = BMKCoorDictionaryDecode(BMKConvertBaiduCoorFrom(CLLocationCoordinate2DMake(37.54380556, 121.314476),BMK_COORDTYPE_GPS));
////    [after addObject:pointAnnotation];
////    pointAnnotation = [[JDOStationAnnotation alloc] init];
////    pointAnnotation.coordinate = BMKCoorDictionaryDecode(BMKConvertBaiduCoorFrom(CLLocationCoordinate2DMake(37.545102, 121.31437),BMK_COORDTYPE_GPS));
////    [after addObject:pointAnnotation];
//    // ===============test===============
//    
