...
@interface JDORealTimeMapController () <NSXMLParserDelegate>{
    int count;
    NSTimer *_timer;
...
- (void) refreshData:(NSTimer *)timer{
    ...
    NSString *soapMessage = [NSString stringWithFormat:GetBusLineStatus_SOAP_MSG,_stationId,_lineId,_lineStatus,_attach];
    ...
    [request addValue:@"http://www.dongfang-china.com/GetBusLineStatusEx" forHTTPHeaderField:@"SOAPAction"];
...
- (void)parserDidEndDocument:(NSXMLParser *)parser{
    if ([_jsonResult isEqualToString:@""]) {    // 说明XML中没有GetBusLineStatusExResult节点，即格式错误
        [JDOUtils showHUDText:@"服务器内部错误" inView:self.view];
    }
}

-(void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *) namespaceURI qualifiedName:(NSString *)qName attributes: (NSDictionary *)attributeDict{
    if( [elementName isEqualToString:@"GetBusLineStatusExResult"]){
        isRecording = true;
    }
}
...
//        }else if(sa.station.isEnd){ // 暂时未用到
//            annotationView.image = [UIImage imageNamed:@"公交未选中"];
//        }else{
